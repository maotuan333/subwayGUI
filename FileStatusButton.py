...
class FileStatusButton(QWidget):
    ...
    def __init__(self, fullpath,
        ...
        # Fullpath can be a str or a list. If more than one file is specified, the button is in 'multiple' mode
        # and checks if all files exist. Otherwise, the button checks for a single file.
        self.fullpath = reformat(fullpath)
        if type(fullpath) == list:
            if len(fullpath) == 1:
                self.fullpath = fullpath[0] # Reduce list of length 1 to str
            else:
                self.multiple = True # Multiple files

        self.button_id = (self.fullpath[0] if self.multiple else self.fullpath).replace('/', '').replace('.','-')
    ...
    def set_file_found(self):
        ...
        self.button.setToolTip(('\n'.join(self.fullpath) if self.multiple else self.fullpath))
    ...
    def set_file_partial(self, results):
        ...
        found_files = [file for file, exists in zip(self.fullpath, results) if exists]
        self.button.setToolTip('\n'.join(found_files))
    ...
    def refresh(self, set_fail=False):
        ...
        if not self.multiple:
            ...
            if glob.glob(self.fullpath):
                self.set_file_found()
                return
        else:
            # Check if all files exist
            results = [glob.glob(file) != [] for file in self.fullpath]
            if all(results):
                self.set_file_found()
                return
            elif any(results):
