...
function Tabs() {
  ...
  // 1E1E1E
  return (
    <div className={`h-10 custom-scrollbar`}>
      <div
        className={`${styles.TabTrack} min-h-10 flex min-w-full items-center custom-scrollbar`}
      >
        {tabs.map((tab) => {
          return (
            <div
              key={tab.id}
              className={`group flex min-w-[170px] flex h-10 z-[50] items-center border-r-[1px] border-primary-gray gap-2 px-2 pr-3 
        ...
        {getActive()?.route !== "/" ? (
          <button
            className="px-3 h-10 bg-transparent hover:bg-primary-gray/[0.4]"
            onClick={() => {
              addTab("/");
            }}
          >
